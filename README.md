...
| 自动判分 | 答案存于后端、下发题目时隐藏，提交后逐题批改并给出**语法解析** |
...
| 听写 | 「🎧 听写」：选课→放音→打出句子→逐词比对高亮，记录最好成绩 |
...
| 数据备份 | 「💾 数据备份」：一键导出所有学习数据为单个 JSON 文件、导入恢复；换设备/防丢失 |
...
| GET | `/api/questions?book=&lessonMin=&lessonMax=&grammar=&type=&limit=&random=1` | 按条件出题（不含答案） |
...
当前为**高质量对齐种子题库**，共约 90 题：第 1 册系统覆盖 24 个语法单元（be 动词、时态、冠词、there be、情态动词、some/any、比较级等），第 2/3/4 册提供代表性语法题（被动、定语从句、虚拟语气、倒装、间接引语、学术词汇辨析等）。

新概念全 4 册 300+ 课，题库设计为**可持续扩充**结构 —— 按上文追加题目即可无限扩展，无需改代码。素材可参考同目录 `../New_Concept_English/` 中的课文 PDF 与音频。
