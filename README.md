...
| 情景对话 | 「💬 情景对话」：**73** 类场景、**1202** 组角色扮演（含 **64** 条多环对话链）；答错台词自动进间隔复习 |
...
| 句型转换 | 「🔀 句型转换」：给出中文→译成英文→依次改一般疑问句、否定句、对句子成分提问（特殊疑问句），另有被动语态、间接引语等高级转换；**可按册 + 语法点筛选**；逐步判分（大小写/标点/缩写不敏感）+ 转换链展示 + 按步骤类型统计；答错的步骤自动进入间隔复习队列，训练量计入每日目标 |
...
| --- | --- | --- | --- | --- | --- |
| 第 1 册 | **72 / 72 课文课**（L1–143 奇数课） | 413 | 75 | 72 | 教材 + 题库 + 听写精读全文完成 |
| 第 2 册 | **96 / 96** | 499 | 102 | 96 | 教材 + 题库 + 精读短文全文完成；听写/背单词/词汇量测试已支持 |
| 第 3 册 | **60 / 60** | **310** | **81** | **60** | 教材 + 题库 + 精读 + 句型转换全书完成 |
| 第 4 册 | **48 / 48** | **240** | **51** | **48** | 教材 + 题库 + 精读 + 句型转换 + 官方词表同步完成 |
| **合计** | **276 课** | **1462 题** | **309** | **276** | |

> **情景对话**为跨册的生活与学术场景内容，不按册划分；现有 **73 类场景 / 1202 组 / 64 条对话链**（`data/dialogues/`）。
