...
function enrichCambridge(cam, book) {
  if (!cam || cam.unit == null) return null;
  const level = cam.level || levelForNceBook(book);
  if (!level) return cam;
  const u = flattenUnits(level).find((x) => x.unit === Number(cam.unit));
  return {
    level,
    unit: Number(cam.unit),
    title: cam.title || (u && u.title) || '',
    titleCn: cam.titleCn || (u && u.titleCn) || '',
    sectionTitleCn: u && u.sectionTitleCn,
    levelTitle: (u && u.levelTitle) || (loadRef().levels[level] && loadRef().levels[level].title),
  };
}

function inferCambridge(transform) {
  if (transform.cambridge && transform.cambridge.unit != null) {
    return enrichCambridge(transform.cambridge, transform.book);
  }
...
function unitsForGrammarTags(book, grammarTags) {
  const level = levelForNceBook(book);
  if (!level || !Array.isArray(grammarTags) || !grammarTags.length) return [];
  const tagSet = new Set(grammarTags);
  const seen = new Set();
  const out = [];
  for (const u of flattenUnits(level)) {
    if (seen.has(u.unit)) continue;
    if ((u.grammar || []).some((g) => tagSet.has(g))) {
      seen.add(u.unit);
      out.push({
        level,
        unit: u.unit,
