...
const { buildDict, normKey, invalidateDictCache } = require('./dict');
...
function extrasSig(ex) {
  return Object.keys(ex).sort().join('\0');
}
...
function buildLookupDict() {
  const ex = loadExtras();
  const sig = extrasSig(ex);
  if (lookupCache && lookupExtrasSig === sig) return lookupCache;
  ...
  for (const [key, meta] of Object.entries(ex)) {
